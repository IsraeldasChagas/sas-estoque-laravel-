...
class RhVagaController extends Controller
{
    ...
    public function destroy(Request $request, int $id)
    {
        ...
        if (! DB::table('rh_vagas')->where('id', $id)->exists()) {
            return response()->json(['error' => 'Vaga não encontrada'], 404)->header('Access-Control-Allow-Origin', '*');
        }

        // Não apaga candidatos; só encerra vaga (soft behavior) para preservar histórico.
        DB::table('rh_vagas')->where('id', $id)->update(['status' => 'encerrada', 'updated_at' => now()]);
