...
const PERMISSOES = {
  ...
  ATENDENTE_CAIXA: {
    sections: ["boasVindas", "minhaConta", "dashboard", "proventos", "reciboAjuda", "fechamento", "fechamentoDash", "fechaTecnica", "reservaMesa", "historicoReservas"],
    canManageUsuarios: false,
    canManageProdutos: false,
    canManageUnidades: false,
    canManageCompras: false,
  ...
  FUNCIONARIO: {
    sections: ["boasVindas", "minhaConta", "dashboard", "proventos", "reciboAjuda", "fechamento", "fechamentoDash", "fechaTecnica"],
    canManageUsuarios: false,
    canManageProdutos: false,
    canManageUnidades: false,
    canManageCompras: false,
...
function applyPermissions() {
  ...
  const patrimonioNavSubmenu = document.getElementById("patrimonioMenu")?.closest(".nav-submenu");
  if (patrimonioNavSubmenu) {
    const temAcessoPatrimonio =
      regras.sections.includes("patrimonioDashboard") ||
      regras.sections.includes("patrimonios") ||
      regras.sections.includes("patrimonioCategorias") ||
      regras.sections.includes("patrimonioMovimentacoes") ||
      regras.sections.includes("patrimonioManutencoes") ||
      regras.sections.includes("patrimonioInventario") ||
      regras.sections.includes("patrimonioRelatorios") ||
...
function navigateTo(section) {
  ...
  else if (section === "patrimonioMovimentacoes") loadPatrimonioMovimentacoes?.().catch(() => {});
  ...
  shell.querySelectorAll("nav a.nav-link[data-section]").forEach((link) => {
    ...
    link.addEventListener("click", async (event) => {
      ...
      if (target === "dashboard") await loadDashboard();
        ...
        await loadEnergiaRelatoriosSection?.();
      } else if (target === "patrimonioDashboard") {
        await loadPatrimonioDashboard?.();
      } else if (target === "patrimonios") {
        await loadPatrimonios?.();
      } else if (target === "patrimonioCategorias") {
        await loadPatrimonioCategorias?.();
      } else if (target === "patrimonioMovimentacoes") {
        await loadPatrimonioMovimentacoes?.();
      } else if (target === "patrimonioManutencoes") {
        await loadPatrimonioManutencoes?.();
      } else if (target === "patrimonioInventario") {
...
function setupReservasMesasModule() {
  ...
  document.getElementById('reservaMesaForm') && document.getElementById('reservaMesaForm').addEventListener('submit', async function(e) {
    ...
    var data = {
      unidade_id: parseInt(unidadeVal, 10),
      mesa_id: parseInt(mesaIdStr, 10),
      nome_cliente: (form.querySelector('[name="nome_cliente"]') && form.querySelector('[name="nome_cliente"]').value.trim()) || '',
      telefone_cliente: (form.querySelector('[name="telefone_cliente"]') && form.querySelector('[name="telefone_cliente"]').value.trim()) || '',
...
/** Soma PDV (totais “sistema” nas linhas) por unidade. `rows` já vem filtrado. */
...
/** Soma PDV por dia (todas as unidades no conjunto filtrado). */
function fechamentoDashSumPdvByDay(rows) {
  const map = new Map();
  (Array.isArray(rows) ? rows : []).forEach((r) => {
    const d = String(r.data_fechamento || "").slice(0, 10);
    if (!d) return;
    const p = fechamentoTotalPdvFromRow(r);
    map.set(d, roundToCurrency((map.get(d) || 0) + p));
  });
  return map;
}
...
function renderFechamentoDashUI(rows) {
  ...
  const byDayPdv = fechamentoDashSumPdvByDay(rows);
  const top3DiasPdv = [...byDayPdv.entries()]
    .map(([data, total]) => ({
      data,
      total,
  ...
  const byUnitPdv = fechamentoDashSumPdvByUnidade(rows);
  const top3UnidadesPdv = [...byUnitPdv.values()]
    .sort((a, b) => b.total - a.total || b.nRegs - a.nRegs)
...
async function loadFechamentoDashSection() {
  if (!document.getElementById("fechamentoDashSection")) {
    showToast(
      "Painel não encontrado: o index.html no servidor precisa incluir a seção fechamentoDashSection. Suba o index.html do repositório.",
      "error",
      12000
    );
    return;
  ...
  try {
    ...
    const tabela =
      st === 503 ||
      /tabela de fechamentos|migrations|não disponível|nao disponivel/i.test(msg);
    showToast(
      tabela
        ? "API sem tabela fechamentos_caixa. No servidor do Laravel execute: php artisan migrate"
        : msg || "Erro ao carregar fechamentos.",
      "error",
      tabela ? 14000 : 8000
    );
  }
...
/** Remove margens/padding inline para manter espaçamento simples no modo de preparo. */
function normalizarEspacamentoModoPreparoHtml(html) {
  const clean = sanitizeFichaTecnicaModoPreparoHtml(html);
  if (!clean) return '';
  const wrap = document.createElement('div');
  wrap.innerHTML = clean;
  const espacoProps = new Set([
    'margin',
    'margin-top',
    'margin-bottom',
    'margin-left',
  ...
  wrap.querySelectorAll('[style]').forEach((el) => {
    ...
    const kept = parts.filter((part) => {
      ...
      return key && !espacoProps.has(key);
    });
...
function modoPreparoHtmlParaEditor(stored) {
  ...
  if (looksHtml) {
    const clean = normalizarEspacamentoModoPreparoHtml(s);
...
function setupFichaTecnicaForm() {
  ...
  const sincronizarModoPreparoOculto = () => {
    ...
    modoHidden.value = normalizarEspacamentoModoPreparoHtml(modoEditor.innerHTML);
    atualizarClassePlaceholderModoPreparo();
  };
    ...
    try {
      ...
      const modo_preparo =
        modoEditor && fichaTecnicaModoPreparoTextoVisivel(modoEditor)
          ? normalizarEspacamentoModoPreparoHtml(modoEditor.innerHTML)
          : '';
